...
class HardinesszoneValidationException implements Exception {
  final List<String> errors;

  HardinesszoneValidationException(this.errors);

  @override
  String toString() => 'HardinesszoneValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://hardinesszone.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [zip] (required) - The zip code to get the USDA Plant Hardiness Zone for [minLength: 5, maxLength: 5]
class HardinesszoneClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'zip': ValidationRule(type: 'string', required: true, minLength: 5, maxLength: 5),
  };

  HardinesszoneClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/hardinesszone',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HardinesszoneValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HardinesszoneValidationException(errors);
    }
  }

  /// Execute a request to the USDA Hardiness Zone API.
  ///
  /// Parameters are validated before sending the request.
  Future<HardinesszoneResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HardinesszoneException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HardinesszoneResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HardinesszoneException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HardinesszoneException('Resource not found');
      } else {
        throw HardinesszoneException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HardinesszoneException) rethrow;
      throw HardinesszoneException('Request failed: $e');
    }
  }
...
/// Exception thrown by the USDA Hardiness Zone API client.
class HardinesszoneException implements Exception {
  final String message;

  HardinesszoneException(this.message);

  @override
  String toString() => 'HardinesszoneException: $message';
}
